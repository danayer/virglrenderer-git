    ...
    steps:
      ...
      - name: Update virglrenderer spec file
        ...
        run: |
          ...
          WORKING_URL=""
          declare -a URL_FORMATS=(
            "https://gitlab.freedesktop.org/virgl/virglrenderer/-/archive/${LATEST_VIRGL_COMMIT}/virglrenderer-${LATEST_VIRGL_COMMIT}.tar.gz"
            "https://gitlab.freedesktop.org/virgl/virglrenderer/-/archive/master/virglrenderer-master.tar.gz"
            "https://gitlab.freedesktop.org/api/v4/projects/virgl%2Fvirglrenderer/repository/archive.tar.gz?sha=${LATEST_VIRGL_COMMIT}"
          )
          ...
          if [ -z "$WORKING_URL" ]; then
            echo "Could not find a working download URL. Defaulting to master branch."
            WORKING_URL="https://gitlab.freedesktop.org/virgl/virglrenderer/-/archive/master/virglrenderer-master.tar.gz"
            echo "Using fallback URL: $WORKING_URL"
          fi
          ...
          # Update Version, Release, and Source lines
          sed -i "s/^Version:.*$/Version:\t$NEW_VERSION/" "$TMP_FILE"
          sed -i "s|^Release:.*$|Release:\t1.git%{shortcommit}%{?dist}|" "$TMP_FILE"
          
          # Replace Source line with direct URL to master archive
          sed -i "s|^Source:.*$|Source:\t\t$WORKING_URL|" "$TMP_FILE"
